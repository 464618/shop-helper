...
function collectLocalProjectData() {
  ...
  const data = {
    ...
    learningProgress: readJson(SYNC_STORAGE_KEYS.learningProgress, {}),
...
function saveProjectDataToLocal(syncDocument) {
  ...
  try {
    ...
    localStorage.setItem(SYNC_STORAGE_KEYS.learningProgress, JSON.stringify(data.learningProgress || {}));
...
function mergeProjectData(localDocument, remoteDocument) {
  ...
  mergedData.learningProgress = { ...(remoteDoc.data.learningProgress || {}), ...(localDoc.data.learningProgress || {}) };
